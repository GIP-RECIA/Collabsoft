...
import static org.springframework.security.cas.web.CasAuthenticationFilter.CAS_STATEFUL_IDENTIFIER;
import static org.springframework.security.cas.web.CasAuthenticationFilter.CAS_STATELESS_IDENTIFIER;
...
public class RememberCasAuthenticationProvider implements AuthenticationProvider, InitializingBean, MessageSourceAware {
  ...
  public Authentication authenticate(Authentication authentication) throws AuthenticationException {
    ...
    if (authentication instanceof UsernamePasswordAuthenticationToken && (!CAS_STATEFUL_IDENTIFIER.equals(authentication.getPrincipal().toString()) && !CAS_STATELESS_IDENTIFIER.equals(authentication.getPrincipal().toString()))) {
    ...
    if (authentication instanceof CasAuthenticationToken) {
      if (this.getKey().hashCode() == ((CasAuthenticationToken) authentication).getKeyHash()) {
        ...
        throw new BadCredentialsException(messages.getMessage("CasAuthenticationProvider.incorrectKey", "The presented CasAuthenticationToken does not contain the expected key"));
      }
    }

    // Ensure credentials are presented
    if ((authentication.getCredentials() == null) || "".equals(authentication.getCredentials())) {
      throw new BadCredentialsException(messages.getMessage("CasAuthenticationProvider.noServiceTicket", "Failed to provide a CAS service ticket to validate"));
    }

    boolean stateless = authentication instanceof UsernamePasswordAuthenticationToken && CAS_STATELESS_IDENTIFIER.equals(authentication.getPrincipal());
  ...
  private CasAuthenticationToken authenticateNow(final Authentication authentication) throws AuthenticationException {
    try {
      final Assertion assertion = this.getTicketValidator().validate(authentication.getCredentials().toString(), getServiceUrl(authentication));
      final UserDetails userDetails = loadUserByAssertion(assertion);
      userDetailsChecker.check(userDetails);
      return new CasAuthenticationToken(this.getKey(), userDetails, authentication.getCredentials(), authoritiesMapper.mapAuthorities(userDetails.getAuthorities()), userDetails, assertion);
